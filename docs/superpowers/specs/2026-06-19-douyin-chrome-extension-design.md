...
- 绕过验证码、登录限制或平台访问控制。
- 直播、直播回放和图文作品批量下载。
- HLS/DASH 分片合并和通用视频格式转码；`ffmpeg.wasm` 仅用于为 ASR 提取和压缩音频。
- 云端同步、服务端爬虫和远程下载代理。
- 保证所有作品均存在无水印可下载源。
...
Service Worker 不能承担持续下载、WASM 音频处理或长轮询任务。扩展按需创建 Offscreen Document，负责把视频源暂存到 OPFS、运行单线程 `ffmpeg.wasm`、上传压缩后的音频、轮询 ASR 任务和调用摘要 Provider。
...
- 使用 `ffmpeg.wasm` 提取单声道、适合 ASR 的压缩音频。
...
3. `ffmpeg.wasm` 提取并压缩为 ASR Provider 支持的音频格式。
...
Manifest 的扩展页 CSP 需要允许本地打包的 WebAssembly：`script-src 'self' 'wasm-unsafe-eval'`。第一期使用单线程 `ffmpeg.wasm`，避免把 SharedArrayBuffer 与跨域隔离要求引入首版。所有 Worker、WASM 和 UI 资源随扩展本地打包，不从 CDN 加载可执行代码。
...
- OPFS 临时媒体生命周期。
- ffmpeg.wasm 音频提取适配层，测试中使用小型固定媒体夹具。
...
- 能收藏博主并低频发现新公开视频。
- 能在 Offscreen Document 中提取音频并通过配置的 ASR Provider 生成全文、时间轴片段和 SRT。
...
6. Offscreen Document、OPFS 和 `ffmpeg.wasm` 音频提取。
