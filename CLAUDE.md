...
## UI 设计规范（Apple 风格，详见 DESIGN.md）

### 色彩系统

| 用途 | 色值 |
|------|------|
| 主 CTA / 焦点环 | `#0071e3`（Apple Blue，**唯一**彩色 accent） |
| 浅色背景 | `#f5f5f7` |
| 深色背景 | `#000000` |
| 主文字（浅背景） | `#1d1d1f` |
| 次文字（浅背景） | `rgba(0,0,0,0.8)` |
| 链接（浅背景） | `#0066cc`；深背景 `#2997ff` |
| 深色卡片 | `#272729`–`#2a2a2d` |
| 卡片阴影 | `rgba(0,0,0,0.22) 3px 5px 30px 0px` |

**禁止**引入第二种彩色 accent；蓝色只用于可交互元素。

### 字体规范

- **≥20px** 用 SF Pro Display；**<20px** 用 SF Pro Text
- fallback：`Helvetica Neue, Helvetica, Arial, sans-serif`
- **所有尺寸**都施加负字间距：56px→-0.28px / 17px→-0.374px / 14px→-0.224px / 12px→-0.12px
- 标题行高压缩到 1.07–1.14；正文行高 1.47；按钮可达 2.41
- 字重范围 300–700，大多数文字在 400/600；700 极少使用

常用字阶：56px(w600/lh1.07) → 40px(w600/lh1.10) → 28px(w400/lh1.14) → 21px(w400/lh1.19) → 17px(w400/lh1.47) → 14px(w400/lh1.29) → 12px(w400/lh1.33)

### 组件规范

**按钮**
- Primary CTA：背景 `#0071e3`，白字，圆角 8px，padding 8px 15px
- Pill 链接（"Learn more"/"Shop"）：透明背景，边框 1px，圆角 **980px**，文字 `#0066cc`
- 禁止：不用渐变背景、不用 800/900 字重

**卡片**
- 圆角 5–8px；**无边框**；阴影仅用上方一种软阴影或完全不用
- 深色卡片用 `#272729` 系列，不用纯黑

**导航**
- 固定高 48px，背景 `rgba(0,0,0,0.8)` + `backdrop-filter: saturate(180%) blur(20px)`
- 链接 12px / w400 / 白色；**玻璃效果不可去除**

### 布局原则

- 基础单元 8px；内容最大宽 ~980px，居中
- 区块用纯色背景交替（黑 / `#f5f5f7`）制造节奏，不用分割线
- 标题行内紧缩，区块间留呼吸空间（≈全屏高）
- 正文**左对齐**；只有标题才居中
- 产品图必须放在纯色背景上，不用纹理/渐变/背景图

### 响应式关键断点

640px（平板双列）→ 1024px（桌面）→ 1440px（大屏留边距）；标题字号 56px→40px→28px 逐级缩小，行高比例保持不变。
